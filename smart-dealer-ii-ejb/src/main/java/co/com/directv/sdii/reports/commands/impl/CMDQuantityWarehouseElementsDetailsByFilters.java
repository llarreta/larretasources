...
import co.com.directv.sdii.reports.commands.CMDQuantityWarehouseElementsDetailsByFiltersLocal;
 ...
 * @see
 */
@Stateless(name = "CMDQuantityWarehouseElementsDetailsByFilters", mappedName = "ejb/CMDQuantityWarehouseElementsDetailsByFilters")
public class CMDQuantityWarehouseElementsDetailsByFilters extends BaseCommand implements
		ICommand,CMDQuantityWarehouseElementsDetailsByFiltersLocal {
	...
	public CMDQuantityWarehouseElementsDetailsByFilters() {
	}
	...
	public <T> List<T> execute(String args) throws BusinessException {
		...
		try {
			...
			strTemp = map.get(CodesBusinessEntityEnum.CMD_REPORT_QUANTITY_WAREHOUSE_ELEMENT_FILTER_MOVCLASS.getCodeEntity());
			if(strTemp != null && strTemp.trim().length() >0)
				filters.setMovClass(strTemp);
			
			strTemp = map.get(CodesBusinessEntityEnum.CMD_REPORT_QUANTITY_WAREHOUSE_ELEMENT_FILTER_MOVTYPE_ID.getCodeEntity());
			if(strTemp != null && strTemp.trim().length() >0)
				filters.setMovTypeId(Long.parseLong(strTemp));
			
			strTemp = map.get(CodesBusinessEntityEnum.CMD_REPORT_QUANTITY_WAREHOUSE_ELEMENT_FILTER_DOCUMENT_ID.getCodeEntity());
			if(strTemp != null && strTemp.trim().length() >0)
				filters.setDocumentId(Long.parseLong(strTemp));
			
			strTemp = map.get(CodesBusinessEntityEnum.CMD_REPORT_QUANTITY_WAREHOUSE_ELEMENT_FILTER_DOCUMENT_CLASS_ID.getCodeEntity());
			if(strTemp != null && strTemp.trim().length() >0)
				filters.setDocumentClassId(Long.parseLong(strTemp));
			
			
			//Ejecucion del metodo que genera el DTO de los detalles de las cantidades de los elementos 
			response = whElementFacade.getQuantityWarehouseElementsDetailsByFilters(filters,null).getQuantityWarehouseElementsDTO();
