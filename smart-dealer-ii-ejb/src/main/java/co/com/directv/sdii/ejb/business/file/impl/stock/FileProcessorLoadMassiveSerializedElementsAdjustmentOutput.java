 ...
 * @see
 */
@Stateless(name="FileProcessorLoadMassiveSerializedElementsAdjustmentOutputLocal",mappedName="ejb/FileProcessorLoadMassiveSerializedElementsAdjustmentOutputLocal")
@TransactionAttribute(TransactionAttributeType.REQUIRED)
@TransactionManagement(TransactionManagementType.CONTAINER)
public class FileProcessorLoadMassiveSerializedElementsAdjustmentOutput extends BasicFileProcessor implements FileProcessorLoadMassiveSerializedElementsAdjustmentOutputLocal {
	...
	private static int POS_COD_BODEGA = 1;
	private static int POS_SERIAL_LINK_ELEMENT = 2;
	
	
	

	public FileProcessorLoadMassiveSerializedElementsAdjustmentOutput(){
		String[] columnTitles = new String[] {
				ApplicationTextEnum.SERIAL.getApplicationTextValue(),
				ApplicationTextEnum.LOCATION_CODE.getApplicationTextValue(),
	...
	public List<FileDetailProcess> doGlobalValidationsAndDropNotValidRecords(
		...
		dealerValids = new ArrayList<Dealer>();
		...
		try{
			...
			if(!transferReason.getAdjustmentType().getCode().equalsIgnoreCase(CodesBusinessEntityEnum.ADJUSTMENT_TYPE_OUTPUT.getCodeEntity())){
				throw new BusinessException("El causal de ajuste debe ser de salida");
			}
			...
			for(Iterator<FileRecordDTO> iterator = fileData.iterator(); iterator.hasNext();){
				...
				try{
					...
					String codeWarehouse = dto.getRowData()[POS_COD_BODEGA];				
	...
	public void processRecord(FileRecordDTO fileRecordDTO)
		...
		try{
			...
			if(!warehouseElement.getWarehouseId().getWhCode().equalsIgnoreCase(codeWarehouse)){
				
				throw new BusinessException("Elemento con serial ["+serialElement+"] no se encuentra en la bodega con código ["+codeWarehouse+"]");
			}
			...
			if(warehouseElement.getWarehouseId().getCountry().getId().longValue()!=adjustment.getCountry().getId().longValue()){
				throw new BusinessException("La ubicación con código ["+codeWarehouse+"] no se encuentra en "+adjustment.getCountry().getCountryName()); 
			}
			...
			checkDealerValid(warehouseElement.getWarehouseId().getDealerId(),codeWarehouse);
			...
			businessAdjustmentElements.adjustmentOutputElementSerialized(UtilsBusiness.copyObject(AdjustmentVO.class, this.adjustment), dto,  this.getUploadFile().getUser(),warehouseAdjusTransit,dtoGenerics);
