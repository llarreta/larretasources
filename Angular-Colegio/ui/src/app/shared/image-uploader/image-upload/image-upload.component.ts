...
export class ImageUploadComponent {
  ...
  @Input('file-max-size') 
  fileMaxSize: number = 2097152; // 2MB
  ...
  fileChange(files) {
    ...
    let filesToUploadNum = 1;//files.length > remainingSlots ? remainingSlots : files.length;
  ...
  private uploadFiles(files, filesToUploadNum) {
    for (let i = 0; i < filesToUploadNum; i++) {
      let file = files[i];
      //console.log(file.size ,    " > " , this.fileMaxSize);
      if(file.size > this.fileMaxSize){
       
        alert("El archivo no debe superar los " + (this.fileMaxSize / 1024 / 1024) + " MB")
      }else{
